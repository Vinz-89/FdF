...
static void initialize_line(t_coord *coord, int *err, int *length)
{
	...
	*length = (coord->dx > coord->dy) ? coord->dx : coord->dy;
}

static void update_line_position(t_coord *coord, int *err)
{
	int e2 = *err * 2;
	if (e2 > -coord->dy)
	{
...
void draw_line(t_fdf *map, t_coord *coord, int color1, int color2)
{
	...
	{
		int color = interpolate_color(color1, color2, (double)step/length);
